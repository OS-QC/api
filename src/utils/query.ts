...
const fxSearchILikeQuery = (
  ...
  if (pAttibutesPermisse.includes(vName)) {
    if (pParam.search.includes(',')) {
      const searchs: string[] = pParam.search.split(',')
      const searchsParse = searchs.map((search: string) => `%${search}%`)
      pWhereStatement.where = {
        ...pWhereStatement.where,
        [vName]: {
          [Op.or]: searchsParse.map((search) => ({ [Op.iLike]: search })),
        },
      }
    } else {
      pWhereStatement.where = {
        ...pWhereStatement.where,
        [vName]: { [Op.iLike]: `%${pParam.search}%` },
      }
    }
  }
...
export const fxSearchILike = (
  ...
  const vAttibutesPermisse = Object.keys(vModelAttributes).filter(
    (key) => vModelAttributes[key].type.key === 'STRING' || vModelAttributes[key].type.key === 'TEXT' 
  )
  if (pParam?.search) {
